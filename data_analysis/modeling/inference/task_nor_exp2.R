...
source("per_setting_exp2.R")
source("fun_infer_nor.R")
df.eve=df.eve[df.eve$time>=0 & df.eve$time<=trial_end,]

vec.sub=c(1,3,4,5,6,7,8,11,13,17,18,19,21,22,24,
          25,27,29,30,32,33,34,36,37,43,44,45,47,48,
          49,52,53,56,58,61,64,65,67,72,76,77,78,80,88,
          89,91,96,97,98,99,100,101,102,108)

vec.curr=c(vec.sub[seq(2,length(vec.sub),9)])
...
for (id in vec.curr){
  ...
  for (i in 1: length(str_sti_type)){
    ...
    save(df.jud.nor,df.post.nor,file=paste("exp2_nor/",id,".Rda",sep="")) 
  }
}