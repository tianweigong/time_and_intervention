...
#exp1>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
load("exp2.Rdata")
df.eve=df.eve %>% subset(time>=0 & time <trial_end)

act_type_set=list(c("A","B","C"),c("A","B","C","D"))
opt_type_set=list(c("A","B","C","E","F","G","I","J","K","N"),
                  c("A","B","C","D","E","F","G","H","I","J","K","L","N"))
noact_type=c('N',"E","F","G","H","I","J","K","L")
...
MyEigRun <-function(sub_filename){
   ...
   fexist=list.files(path = paste('./',new_fld,sep = ""),pattern = "\\.Rda$")
   if (sub_filename %in% fexist){
      load(paste(new_fld,'/',sub_filename,sep=""))
      if (length(lis.record)==nrow(df.jud.nor)){return()}
      beginfrom=length(lis.record)+1
   }else{beginfrom=1}
   
   for (i in beginfrom:nrow(df.jud.nor)){
      ####
      ...
      act_exist=df.eve.sub %>% subset(trName==df.jud.nor$trName[i-1] & time<(wd-1)*wd_len &act==1)
      
      if (nrow(act_exist)==6){#preload some common patterns #&&myblc=="" && df.jud.nor$ent_nor[i]==df.jud.nor$ent_nor[i-5]
         lis.record[[i]]="end"
         next
      }
      ...
      if (wd==45){#preload some common patterns
         lis.record[[i]]="end"
         next
      }
   
      act_ongoing=act_exist %>% subset(time>(wd-maxtrack+2)*wd_len) #too early actions can not affect the EIG later
