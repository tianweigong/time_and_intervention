...
MyFit<-function(v){
  ...
  if (par_num>1){
    par_ini=lapply(as.list(rep(NA,ininum)),function(x) c(1,-10)+rnorm(2,0,0.01))
  }
  ...
  save(x,y,BIC,file = paste("softmax_act_exp2/",mod,".Rda",sep = ""),version=2)
  ...
  save(x,y,BIC,idd,file = paste("softmax_act_exp2/",mod,".Rda",sep = ""),version=2)
  ...
  save(x,y,BIC,idd,cv,cv_logli,file = paste("softmax_act_exp2/",mod,".Rda",sep = ""),version=2)
  ...
  save(x,y,BIC,cv,cv_logli,idd,idd_cv,file = paste("softmax_act_exp2/",mod,".Rda",sep = ""),version=2)
}
...
idmtx_cv=matrix(NA,nrow = 95,ncol=3)
idmtx=matrix(NA,nrow = 95,ncol=3)
load("softmax_act_exp2/eig.Rda")
for (m in 1:length(idd_cv)){
  idmtx_cv[m,1]=-sum(idd_cv[[m]][["CV"]])
  idmtx[m,1]=idd[[m]][["BIC"]]
}
load("softmax_act_exp2/eigecc.Rda")
for (m in 1:length(idd_cv)){
...
t.test(x1$accLink,x2$accLink)
t.test(x2$accLink,x3$accLink)
t.test(x1$accLink,x3$accLink)
